...
# Singular perturbation of epsilon Roz-Mac Type II model
...
@with_kw mutable struct RozMacParCI
    r = 2.0
    k = 3.0
    a = 1.1
    c = 0.1
...
par_rozmacCI = RozMacParCI()

function roz_mac_C_I!(du, u, p, t,)
    @unpack r, k, a, c = p
    R = u
    du = r * R * (1 - R / k) - a * R * c
    return
...
function roz_mac_C_I(u, par)
    du = similar(u)
    roz_mac_C_I!(du, u, par, 0.0)
    return du
end
...
function eq_C_I(p)
    @unpack r, a, k, c = p
    eq_I = k * (-a * c + r) / r
    return vcat(0.0, eq_I)
end

#Jacobian analysis as change Consumer parameter
#One dimensional model so bifurcations possible are saddle-node, transcritical, pitchfork
# Prediction should only be transcritical

Calculus.simplify(differentiate("r * u1 * (1 - u1 / k) - a * u1 * c", :u1))

function roz_mac_I_C_eigen(p)
    @unpack r, a, k, c = p
    eq = eq_C_I(p)
    eig = [0.0, 0.0]
    for i in 1:2
        R = eq[i]
        eig[i] = (r * (1 - R / k) + r * R * -(1 / k)) - a * c
    end
    return hcat(eq,eig)
end

roz_mac_I_C_eigen(RozMacParCI(c = 1.5))

function roz_mac_I_C_bifurc(par, cvals)
    eq = Any[]
    eig = Any[]

    for (ci, cval) in enumerate(cvals)
        par.c = cval
        equ_eig = roz_mac_I_C_eigen(par)
        push!(eq, equ_eig[:, 1])
        push!(eig, equ_eig[:, 2])
    end
    return [transpose(convert(Array, VectorOfArray(eq))), transpose(convert(Array, VectorOfArray(eig)))]
...
function find_trans(cvals)
    data = hcat(cvals, roz_mac_I_C_bifurc(par_rozmacCI, cvals)[2][:,1])
...
let
    ...
    after_trans_cvals = 1.82:0.001:2.281
    sing_pert = figure()
    plot(before_trans_cvals, roz_mac_I_C_bifurc(par_rozmacC, before_trans_cvals)[1][:,1], color = "black", linestyle = "dashed")
    plot(after_trans_cvals, roz_mac_I_C_bifurc(par_rozmacC, after_trans_cvals)[1][:,1], color = "black", linestyle = "solid")
    plot(before_trans_cvals, roz_mac_I_C_bifurc(par_rozmacC, before_trans_cvals)[1][:,2], color = "black", linestyle = "solid")
    plot(after_trans_cvals, roz_mac_I_C_bifurc(par_rozmacC, after_trans_cvals)[1][:,2], color = "black", linestyle = "dashed")
    plot(fullcvals, fill(con_iso(RozMacPar(e = 0.5)), length(fullcvals)))
    ylabel("Resource")
    xlabel("Consumer")
    return sing_pert
end
...
#Question what is type II response doing to the dynamics?
# so no canard in type I, so is epsilon doing anything here
